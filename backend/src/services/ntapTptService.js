...
function calculateNtapPayment(params) {
  ...
  const { deviceCost, drgCode, drgPayment: providedDrgPayment } = params;
  ...
  const ntapPercentage = ntapData.ntapPercentage || config.ntap.percentage;
  const maxCap = ntapData.maxNtapCap || config.ntap.maxCap;
...
function checkNtapEligibility(params) {
  ...
  const costThreshold = drgPayment * (ntapData.costThresholdMultiplier || config.ntap.costThresholdMultiplier);
...
function getApprovedNtapTechnologies() {
  ...
  return {
    ...
    totalCount: ntapData.technologies?.length || 0,
  };
}
...
function calculateTptPayment(params) {
  ...
  const { deviceCost, apcCode, packagedPayment: providedPackagedPayment } = params;
...
function checkTptEligibility(params) {
  ...
  const now = new Date();
  const yearsOld = (now - fdaDate) / (365.25 * 24 * 60 * 60 * 1000);
  const maxDuration = tptData.maxPassThroughDuration || config.tpt.maxPassThroughDuration;
  
  const newnessCriteria = {
    criterion: 'Newness',
    description: `Recent FDA approval (within ${maxDuration}-year window)`,
    met: yearsOld <= maxDuration,
    details: yearsOld <= maxDuration
      ? `Approved ${yearsOld.toFixed(1)} years ago - within ${maxDuration}-year window`
  ...
  const costSignificant = apcPayment > 0 && deviceCost > (apcPayment * 0.15);
  
  const costCriteria = {
    criterion: 'Cost Significance',
    description: 'Device cost represents significant portion of procedure cost',
    met: costSignificant,
    details: apcPayment > 0
      ? `Device cost ($${deviceCost.toLocaleString()}) is ${((deviceCost / apcPayment) * 100).toFixed(1)}% of APC payment`
...
function getApprovedTptTechnologies() {
  ...
  return {
    ...
    maxDuration: tptData.maxPassThroughDuration || config.tpt.maxPassThroughDuration,
    technologies: tptData.technologies,
    totalCount: tptData.technologies?.length || 0,
  };
}
...
function getAvailableDrgs() {
  loadData();
  if (!ntapData.drgBasePayments) return [];
...
function getAvailableApcs() {
  loadData();
  if (!tptData.apcBasePayments) return [];
